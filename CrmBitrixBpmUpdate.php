...
class CrmBitrixBpmUpdate extends CCrmCompany
{
    ...
    public static function contragenUpdate($verifity = array())
    {
        ...
        /*if ($marginMetkaAdress > 0 and $addMetkaAdress != 0) {
            $verifity["adress"] = array_reverse($verifity["adress"]);
            $adressAdd = array_slice($verifity["adress"], -$marginMetkaAdress);

            foreach ($adressAdd as $num => $adrval) {
                $addAddress = array(
                    "__type" => "Terrasoft.Nui.ServiceModel.DataContract.InsertQuery",
                    "RootSchemaName" => "AccountAddress",
                    "OperationType" => 1,
            ...
            if ($addMetkaAdress > 0) {
                $verifity["adress"] = array_reverse($verifity["adress"]);
                $upAdress = array_diff_key($verifity["adress"], $adressAdd);
                foreach ($upAdress as $num => $adrval) {

                    $addAddress_update = array(
                        '__type' => 'Terrasoft.Nui.ServiceModel.DataContract.UpdateQuery',
                        'RootSchemaName' => 'AccountAddress',
                        'QueryType' => 1,
                        'ColumnValues' =>
                    ...
                    if (!empty($adrval)) {
                        $batshContragent[] = $addAddress_update;
                    }
                }
            }
        }
        if ($addMetkaAdress == 0 and $marginMetkaAdress > 0) {
            foreach ($verifity["adress"] as $num => $adrval) {
                $addAddress = array(
                    "__type" => "Terrasoft.Nui.ServiceModel.DataContract.InsertQuery",
                    "RootSchemaName" => "AccountAddress",
        ...
        if ($addMetkaAdress == $updateMetkaAdress) {
            foreach ($verifity["adress"] as $num => $adrval) {
                $addAddress_update = array(
                    '__type' => 'Terrasoft.Nui.ServiceModel.DataContract.UpdateQuery',
                    'RootSchemaName' => 'AccountAddress',
        ...
        /*if ($marginMetkaBank > 0 and $addMetkaBank != 0) {
            $bankAdd = array_slice($verifity["bank"], -$marginMetkaBank);
            foreach ($bankAdd as $n => $b) {
                $bank = array(
                    "__type" => "Terrasoft.Nui.ServiceModel.DataContract.InsertQuery",
                    "RootSchemaName" => "AccountBillingInfo",
                    "OperationType" => 1,
            ...
            if ($addMetkaBank > 0) {
                $verifity["bank"] = array_reverse($verifity["bank"]);
                $upBank = array_diff_key($verifity["bank"], $bankAdd);
                foreach ($upBank as $n => $b) {
                    $bank_update = array(
                        '__type' => 'Terrasoft.Nui.ServiceModel.DataContract.UpdateQuery',
                        'RootSchemaName' => 'AccountBillingInfo',
                        'QueryType' => 1,
                    ...
                    if (!empty($b) and !empty($b["RQ_BANK_NAME"])) {
                        $batshContragent[] = $bank_update;
                    }
                }
            }
        }
        if ($addMetkaBank == 0 and $marginMetkaBank > 0) {
            foreach ($verifity["bank"] as $n => $b) {
                $bank = array(
                    "__type" => "Terrasoft.Nui.ServiceModel.DataContract.InsertQuery",
                    "RootSchemaName" => "AccountBillingInfo",
        ...
        if ($addMetkaBank == $updateMetkaBank) {
            foreach ($verifity["bank"] as $n => $b) {
                $bank_update = array(
                    '__type' => 'Terrasoft.Nui.ServiceModel.DataContract.UpdateQuery',
                    'RootSchemaName' => 'AccountBillingInfo',
